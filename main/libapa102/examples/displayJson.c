...
int main()
{
  ...
  uint8_t colorsForStrip[NB_SECTORS][3];
  uint8_t colorsForStrip2[NB_SECTORS][3];
  ...
  for (int i = 0; i < NB_COLOR_POINTS; i++)
  {
    ...
    if(rIndex>=0)
    {
      colorsForStrip[rIndex][0] = colorPoints[2];
      colorsForStrip[rIndex][1] = colorPoints[3];
      colorsForStrip[rIndex][2] = colorPoints[4];
    }
    else
    {
      rIndex = -rIndex;
      colorsForStrip2[rIndex][0] = colorPoints[2];
      colorsForStrip2[rIndex][1] = colorPoints[3];
      colorsForStrip2[rIndex][2] = colorPoints[4];
    }
  }
